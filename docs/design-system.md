...
| `primary-500` | `#6366F1` | **Primary brand** — buttons, links, current user |
...
| `accent-500` | `#A8680B` | **Main accent** — highlights, featured items |
...
Warmer than zinc, pairs well with indigo for a professional feel. Use `slate-*` everywhere — **never `gray-*` or `zinc-*`**.
...
Key animations:
- **Layout shifts**: Framer Motion `layout` prop — elements slide smoothly when data changes.
...
**Reduced motion**: `globals.css` includes a `prefers-reduced-motion` media query that disables all animations and transitions for users who request it. No per-component opt-out needed.
...
Reusable patterns from primitives. App-agnostic — no domain awareness.
...
**AppShell** — Top-level layout, switches MobileNav + FloatingFab (mobile) vs. SidebarNav (desktop).
```tsx
<AppShell>
  {children}
</AppShell>
...
10. **No raw Tailwind color classes in domain/page code** — use semantic tokens (`primary-*`, `accent-*`, `slate-*`).
11. **User-facing text via `next-intl`** — English is the default locale. Code uses English translation keys; actual labels are resolved from translation files at runtime.
